// HTTP Basic Auth for /admin/* — credentials default to sloth / slothlabsorg123,
// overridable via the ADMIN_AUTH_USER / ADMIN_AUTH_PASS env vars (shared with
// /stats edge auth and the news-save CMS function). Runs on Netlify's edge
...
export default async (request: Request): Promise<Response | void> => {
  ...
  const deny = () =>
    new Response('Authentication required', {
      status: 401,
      headers: {
        'WWW-Authenticate': 'Basic realm="SlothLabs Admin", charset="UTF-8"',
