...
|---------|-------------|
| **NODOS (10)** | The flow has 10 nodes: 2 I/O, 1 loader, 1 chunker, 1 embedder, 1 store, 1 retriever, 1 LLM, 1 prompt, and 1 citations |
| **ARISTAS (11)** | 11 connections; the `chat_input` node feeds two routes (to `retriever` and to `prompt`); `retriever` also feeds two destinations (`prompt` and `citations`) |
| **NODO DE ENTRADA** | `chat_input` (type `io.input`) — RAGorbit derives `deploymentTarget: chat-service` from this node |
| **NODO(S) DE SALIDA** | `chat_output` (type `io.output`) — sole node with no outgoing edges |
