...
function getTimeLeft() {
  ...
  if (diff <= 0) return { days: 0, hours: 0, text: 'Live now', isLive: true }
  const days = Math.floor(diff / (1000 * 60 * 60 * 24))
  const hours = Math.floor((diff % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60))
  if (days > 0) return { days, hours, text: `${days} day${days !== 1 ? 's' : ''}, ${hours} hr${hours !== 1 ? 's' : ''} to go`, isLive: false }
  return { days: 0, hours, text: `${hours} hour${hours !== 1 ? 's' : ''} to go`, isLive: false }
}
...
export function LaunchBanner({ variant = 'banner' }: { variant?: 'banner' | 'badge' | 'subtle' }) {
  ...
  const [timeLeft, setTimeLeft] = useState({ days: 0, hours: 0, text: '', isLive: false })
  ...
  // LIVE state — use green-ish color with pulse animation
  if (timeLeft.isLive) {
    if (variant === 'badge') {
      return (
